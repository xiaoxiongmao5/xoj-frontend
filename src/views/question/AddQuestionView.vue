<template>
  <div id="addQuestionView">
    ...
    <a-form :model="form" label-align="left">
      ...
      <a-form-item field="tags" size="medium" label="标签">
...
const loadData = async () => {
  ...
  const res = await Service.getQuestionGet(id as any);
  if (res.code === 0) {
...
const doSubmit = async () => {
  ...
  // 区分更新还是创建
  if (updatePage) {
    const res = await Service.postQuestionUpdate(form.value);
    if (res.code === 0) {
    ...
    const res = await Service.postQuestionAdd(form.value);
    if (res.code === 0) {
